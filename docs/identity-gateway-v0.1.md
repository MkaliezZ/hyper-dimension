...
当前本地 Agent 原生教师 API 会读取活动的 SQLite 教师策略，阻断同岛其他教师的班级名单、学生档案、归档和展示写入；旧模拟接口在没有策略时仍兼容测试夹具。这是过渡授权，不是正式的教师—班级分配关系。生产必须把本章所述关系迁入 PostgreSQL，并在业务写事务中再次校验，不能将“拥有教师令牌”视为对整个岛的学生有权限。
...
**教师—班级关系增量（2026-09-26）**：新增 `migrations/0002_teacher_class_assignments.sql`，建立岛内班级、教师分配、撤销版本和带班级引用的授权审计；`scripts/migrate_identity.py` 按 0001、0002 顺序执行并核对校验和。`IdentityStore.active_teacher_class` 供教师 API 每次访问查询，结果再与 SQLite 活动教师策略取交集。PostgreSQL 17 实库 CI 已验证 0002 迁移、授权撤销与教师 API 实时拒绝（133 项通过）；本机未配置 PostgreSQL 测试库。MCP 工具逐对象班级授权已接线并通过本地负例；OIDC 教师 API 与远程 MCP 漏配实时班级关系源时会拒绝启动，HTTP MCP 的实库端到端复核已通过：撤销班级分配后教师 API 与已认证的 MCP 工具均拒绝读取；本轮 PostgreSQL 17 CI 为 134 项通过。学生正式分配、监护核验、业务数据 PostgreSQL 迁移及生产开户仍未完成。
