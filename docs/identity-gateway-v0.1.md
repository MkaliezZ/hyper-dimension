...
**教师—班级关系增量（2026-09-26）**：新增 `migrations/0002_teacher_class_assignments.sql`，建立岛内班级、教师分配、撤销版本和带班级引用的授权审计；`scripts/migrate_identity.py` 按 0001、0002 顺序执行并核对校验和。`IdentityStore.active_teacher_class` 供教师 API 每次访问查询，结果再与 SQLite 活动教师策略取交集。当前本机未配置 PostgreSQL 测试库，增量实库 CI 待验收。学生正式分配、监护核验、业务数据 PostgreSQL 迁移、MCP 工具逐对象班级授权及生产开户仍未完成。
