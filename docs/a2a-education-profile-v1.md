...
状态：**结构化契约候选冻结；只有本机两项只读 A2A 端点，尚无公网/生产端点**。这是 Hyper Dimension 的业务层，A2A v1 负责 Agent 发现、消息、任务状态和传输；MCP 负责教师 Agent 调用本地或云端业务工具。正式对外开放前，还需身份网关、权限实装与跨 Agent 联调。
...
当前另有[本地 A2A 只读原型](local-a2a-read-prototype.md)：官方 SDK 路由只提供教师档案读取和当前已发布成果读取，并在 Task Artifact 中用[只读内容 Schema](../src/hyper_dimension/schemas/hd-education-a2a-read-content-v1.2.schema.json)提供实际数据。它仅对一个本地绑定教师/岛使用静态令牌，未完成生产身份网关和第 2—5 步的完整实现。其他九项操作（尤其五个学生 Agent 操作）目前只有契约，不在 Agent Card 中宣称可用。
