...
远程教师 MCP 要求配置实时班级关系源，否则拒绝启动；每个带学生或班级引用的工具调用前查询当前 PostgreSQL 教师—班级分配，并与本地活动教学策略交叉校验；撤销分配后，已有 Agent 工作负载令牌也不能继续读取该班级。公开成果读取只返回仍被许可的已发布卡片。当前业务资料仍为 SQLite 模拟数据，正式学生关系与同事务授权尚待迁移。
