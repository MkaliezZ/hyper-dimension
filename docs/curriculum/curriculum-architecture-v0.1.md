...
每单元按以下顺序生产：核对教材元数据与来源 → 选能力节点 → 写原创大问题 → 定义独立终结任务及成功证据 → 逆向设计情境导入、听读获取、语言发现、有支持的输出、独立输出、新情境迁移 → 提供 L0—L3 分层脚手架 → 收集证据并按教师预授权策略自动评测、批准，异常交教师复核。日常 3—5 分钟形成性练习、周综合任务、月证据报告和学期目标调整均按学校规则和低龄负担调整。
...
教师 Agent 在教师预授权策略内实时生成题目、答案、量规与计划，服务端逐次核验质量门；常规作答自动评分、批准和生成可见报告，异常交教师复核。一次测试只能给证据支持的临时起点，证据不足则补测。版权与来源不明的课本扫描页、音视频不进入公库。
